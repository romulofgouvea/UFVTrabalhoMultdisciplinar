...
public abstract class BaseFragments extends Fragment {
    private static final String TAG = "RFG BaseFragments";
...
//        Log.d("SeuGuia - Leopoldina", "startTask: " + cod);
        View view = getView();
        if (view == null) {
...
//            Log.d("SeuGuia - Leopoldina", "task onPreExecute()");
...
//                Log.e("SeuGuia - Leopoldina", e.getMessage(), e);
                r.exception = e;
            }
            return r;
        }
...
//            Log.d("SeuGuia - Leopoldina", "task onPostExecute(): " + result);
            try {
                if (result != null) {
...
//        Log.d("SeuGuia - Leopoldina", "closeProgress()");
    ...
    public void replaceFragment(int idContainer, Fragment frag) {//MUDAR O FRAGMENT NA MAIN
        getActivity().getSupportFragmentManager().beginTransaction().replace(idContainer, frag, "TAG").commit();
    }

    public boolean isEmpty(EditText etText) {
        if (etText.getText().toString().trim().length() == 0) {
            etText.setError("Este campo não pode ser vazio!");
            return true;
        } else {
            return false;
        }
